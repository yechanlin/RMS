...
if (!(Test-Path "README.md")) {
    Write-Host "Please run this script from the RMSr root directory" -ForegroundColor Red
    exit 1
}

# Start backend server
Write-Host "Starting Django backend server..." -ForegroundColor Yellow
Start-Process PowerShell -ArgumentList "-NoExit", "-Command", "Set-Location 'D:\Hackathon\RMS\backend'; .\venv\Scripts\Activate.ps1; python manage.py runserver 8000"
...
Start-Process PowerShell -ArgumentList "-NoExit", "-Command", "Set-Location 'D:\Hackathon\RMS\frontend'; npm start"
